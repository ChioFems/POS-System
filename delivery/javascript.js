...
$(document).ready(function(){
	...
	$(document).on('click','.remove_row',function(){
		...
		var final_amount = $('#final_total_amount').text().replace("TZS","");
		...
		if(isNaN(result_amount)){
			...
			if(total_product_amount == ""){
				...
				$('#total_amount').text('TZS '+minus_total);
			}else{
				$('#final_total_amount').text('TZS 0.00');
			}
			
		}else{
			$('#final_total_amount').text('TZS '+result_amount);
		}
	...
	function final_total(count){
		...
		$('#final_total_amount').text('TZS '+final_product_amount);
	}
